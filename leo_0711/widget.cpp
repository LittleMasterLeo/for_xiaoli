...
void Widget::ClientRecvData()
{
    ...
    */
    ...
    while(sockfd->bytesAvailable() >= sizeof(load_data))
    {
        int recvSize = sockfd->bytesAvailable();
        qDebug()<<"recvsize: "<<recvSize;
        //int recvRe = sockfd->read(recvMsg,sizeof(recvMsg));
        int recvRe = sockfd->read((char *)&load_data,sizeof(load_data));
        if(recvRe == -1)
        {
            QMessageBox::information(this, "QT网络通信", "接收服务端数据失败！");
            return;
        }
        qDebug()<<"recvre: "<<recvRe;
        //memcpy(load_data,recvMsg,sizeof(load_data));
        if(load_data.emg_data_ready == 1){
            memcpy(emg_set.emg_flt,load_data.emg_data,sizeof(emg_set.emg_flt));
            emg_set.test_new();
        }
        if(load_data.robot_data_ready == 1){
            memcpy(rbt_set.force_data,load_data.force_data,sizeof(rbt_set.force_data));
            memcpy(rbt_set.vector_data,load_data.vector_data,sizeof(rbt_set.vector_data));
            rbt_set.show_data();
        }

        //memcpy(emg_set.emg_flt,recvMsg,sizeof(emg_set.emg_flt));
        //qDebug()<<"fltsize: "<<sizeof(emg_set.emg_flt);
        //emg_set.test_new();
        break;
    }
/*
...
void Widget::on_robot_button_clicked()
{
    rbt_set.show();
}
