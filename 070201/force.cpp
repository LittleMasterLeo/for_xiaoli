...
/////////////////////////////////////////////////////
/* Get time function (create by leo) */
char sTemp[30] = {0};
char* get_Sys_Time(char *sTemp)
{
     struct timeval tv;
     ...
     sprintf(sTemp, "%04d/%02d/%02d/%02d:%02d:%02d:%03ld.%03ld", pTime->tm_year+1900,
...
{	
	...
	double apar[6] = {0,-0.24365,-0.21325,0,0,0};
	double dpar[6] = {0.15190,0,0,0.11235,0.08535,0.0819};
...
//postion error for EA
double X0,X1,Y0,Y1=0.0;
//int i,j;
...
double calculate_distance(double x0, double y0, double x1, double y1, double deg)
	{
		//double distance;
		distanceV = sqrt(pow(y1-y0,2)+pow(x1-x0,2))*sin(deg);
		return distanceV;
	}


void forceControl(UrDriver *ur3, std::condition_variable *rt_msg_cond_, int run_time, int force_mode, double user_parameters[6], double f_ref, double t_ref)
{
	pthread_t forceID;
	startFT(&forceID);	
	
	/////////////////////////////////////////////////////////////////
		/* Get time (create by leo) */
		get_Sys_Time(sTemp);
	/////////////////////////////////////////////////////////////////
	...
	char file_name_forcelog[128]={0};
	strcpy(file_name_forcelog,"/home/xiaoli/Documents/xiaoli/data_xiaoli/logs/forcelog");
	strcat(file_name_forcelog,sTemp);
	strcat(file_name_forcelog,".csv");
	forcelog.open(file_name_forcelog,std::ofstream::app);
	
    //    forcelog.open("/home/xiaoli/Documents/xiaoli/data_xiaoli/logs/forcelog.csv", std::ofstream::app);
	std::ofstream leo_test;
	//	leo_test.open("/home/xiaoli/Documents/xiaoli/data_xiaoli/logs/leo_test.txt", std::ofstream::app);
	char file_name_leo[128]={0};
	...
	double bias_tool_WF[3] = {0, 0, -1.6449};//-0.5886[N] is the calculated gravitational force on the end-effector tool in world frame ('_WF').
	//double bias_tool_WF[3] = {-2500, -2500, -1.6449};
	double bias_tool_TF[3]; //Gravitational force of end-effector tool in tool frame ('_TF').

	double radius_current;
	double radius_min = 0.2;//unit [m] 
	double radius_max = 0.45;
	double height_min = -0.2;
	...
	double start_time = ur3->rt_interface_->robot_state_->getTime();
	std::vector<double> sq = ur3->rt_interface_->robot_state_->getQActual();
	...
	Kp = 0.004;// Prefered between [0.004-0.005]
	//Ki = 0.000002; // Not prefered due to overshoot behaviour.
	//Kd = 0.0001; // Not prefered due to noise amplification
	
	Kp_T = 0.12;// Prefered between [0.1-0.12]
	//Ki_T = 0.000005; // Not prefered due to steady-state error.
	...
	//double Orig[2] = {0.0498605, -0.244383};
	//double deg_1, deg_2;
	//double distance;
	...
	while(i<iter)
	{
		...
		double time_stamp = ur3->rt_interface_->robot_state_->getTime();
		double elaps_time = time_stamp-start_time;
				  
		std::vector<double> q = ur3->rt_interface_->robot_state_->getQActual();
		//std::vector<double> qd = ur3->rt_interface_->robot_state_->getQdActual();
		//std::vector<double> qdd_target = ur3->rt_interface_->robot_state_->getQddTarget()
		//std::vector<double> tcp_speed = ur3->rt_interface_->robot_state_->getTcpSpeedActual();
		...
		// postion error
		X0 = 0.0498605;Y0 = -0.244383;
		X1 = -0.149253;Y1 = -0.531966;
		
		// deg_1
		double ab =sqrt(pow((X0-X1),2.0)+pow((Y0-Y1),2.0));
		double ac =sqrt(pow((X0-X0),2.0)+pow((Y0-Y1),2.0));
		double bc =sqrt(pow((X0-X1),2.0)+pow((Y1-Y1),2.0));

		double cos_B = (pow(bc,2.0)+pow(ab,2.0)-pow(ac,2.0))/(2*ab*bc);
   		double sin_B = sqrt(1-pow(cos_B,2.0));

		// distance of point to line
		double as = sqrt(pow((X0-gsl_vector_get(O,0)),2.0)+pow((Y0-gsl_vector_get(O,1)),2.0));
		double bs = sqrt(pow((X1-gsl_vector_get(O,0)),2.0)+pow((Y1-gsl_vector_get(O,1)),2.0));
   
    	double cos_A = (pow(ab,2.0)+pow(as,2.0)-pow(bs,2.0))/(2*ab*as);
		double sin_A = sqrt(1-pow(cos_A,2.0));
        distanceV = as*sin_A;
		distance[0] = distanceV * sin_B;
		distance[1] = distanceV * cos_B;

		deg_1 = get_slope(X0,Y0,X1,Y1);
		deg_2 = get_slope(X0,Y0,gsl_vector_get(O,0),gsl_vector_get(O,1));
		if(deg_2 >= deg_1)
		{
			//distanceV = calculate_distance(X0,Y0,gsl_vector_get(O,0),gsl_vector_get(O,1),deg_2-deg_1);
			
			//distance[0] = distanceV*sin(deg_2-deg_1);
			//distance[1] = -distanceV*cos(deg_2-deg_1);
			distance[0] = distance[0];
			//distanceV*sin(deg_1);
			//*sin(fabs(deg_2-deg_1));
			distance[1] = -distance[1];
			// -distanceV*cos(deg_1);
			//*cos(fabs(deg_2-deg_1));
		}
		else
		{
			//distanceV = calculate_distance(X0,Y0,gsl_vector_get(O,0),gsl_vector_get(O,1),deg_1-deg_2);
			//distance[0] = -distanceV*sin(deg_1-deg_2);
			//distance[1] = distanceV*cos(deg_1-deg_2);
			distance[0] = -distance[0];
			//*sin(fabs(deg_1-deg_2));
			distance[1] =  distance[1];
			//*cos(fabs(deg_1-deg_2));
		}
		//distance[0] = distanceV*sin(deg_1);
		//distance[1] = distanceV*sin(-M_PI_2-deg_1);
   		...
   		//FORCE ERROR UPDATES
   		if(fabs(forces[0]) < 1.5 && fabs(forces[1]) < 1.5 && fabs(forces[2]) < 1.5) // Dead-band filter, cut-off at 1N
   		{
		...
		if(fabs(error_Tx) > 5 || fabs(error_Ty) > 5 || fabs(error_Tz) > 5)
		{
			std::cout << "============================= STOPPING! ============================" << std::endl;
			ur3->halt();
		...
		//SAFETY MECHANISM 
		if(fabs(u_Fx) > 20 || fabs(u_Fy) > 20 || fabs(u_Fz) > 20)
		{
			std::cout << "============================= STOPPING! ============================" << std::endl;
			ur3->halt();
		...
		{
			for (int j = 0; j<2; j++) //Random torque is disabled due to safety conserns
			{
				...
				references[j] = set_k*distance[j]; // 100k
				//references[j] = 20*distanceV;
		...
		{	
			...
			for (int j = 0; j<6; j++)
			{
				...
				{
					vw[j] *= (1 + (user_parameters[j]-0*distance[j]));
					//vw[j] = vw[j]+0.5*distance[j];
				}
			}
		}
		...
		ur3->setSpeed(speed[0], speed[1], speed[2], speed[3], speed[4], speed[5], 10);
		...
		forcelog <<sTemp<<","<< elaps_time << "," << speed[0] << "," << speed[1] << "," << speed[2] << "," 
		<< speed[3] << "," << speed[4] << "," << speed[5] << "," << q[0] << "," << q[1] << "," 
		<< q[2] << "," << q[3] << "," << q[4] << "," << q[5] << "," << rawFTdata[0] << "," 
		<< rawFTdata[1] << "," << rawFTdata[2] << "," << rawFTdata[3] << "," << rawFTdata[4] << ","
		<< rawFTdata[5] << "," << forces[0] << "," << forces[1] << "," << forces[2] << "," 
		<< torques[0] << "," << torques[1] << "," << torques[2] << "," << error_Fx << "," 
		<< error_Fy << "," << error_Fz << "," << error_Tx << "," << error_Ty << "," 
		<< error_Tz << "," << u_Fx << "," << u_Fy << "," << u_Fz << "," << u_Tx << "," 
		<< u_Ty << "," << u_Tz << "," << bias_force[0] << "," << bias_force[1] << "," 
		<< bias_force[2] << "," << bias_tool_TF[0] << "," << bias_tool_TF[1] << "," 
		<< bias_tool_TF[2] << "," << gsl_vector_get(O,0) << "," << gsl_vector_get(O,1) << ","
		<< gsl_vector_get(O,2) << "," << disturbances[0] << "," << disturbances[1] << "," 
		<< disturbances[2] << "," << buoyancy_vector_TF[0] << "," << buoyancy_vector_TF[1] 
		<< "," << buoyancy_vector_TF[2] << "," << radius_datalog[0] << "," << radius_datalog[1] << "," 
		<< radius_datalog[2] << "," << height_datalog[0] << "," << height_datalog[1] << "," 
		<< height_datalog[2] << "," << gsl_matrix_get(R,0,0) << "," << gsl_matrix_get(R,0,1) << ","
		<< gsl_matrix_get(R,0,2) << "," << gsl_matrix_get(R,1,0) << "," << gsl_matrix_get(R,1,1) 
		<< "," << gsl_matrix_get(R,1,2) << "," << gsl_matrix_get(R,2,0) << "," 
		<< gsl_matrix_get(R,2,1) << "," << gsl_matrix_get(R,2,2) << "," << "\n";
		
		//leo_test data collecting
		leo_test<<i<<" "<<gsl_vector_get(O,0)<<" "<<gsl_vector_get(O,1)<<" "<<distance[0]<<" "<<distance[1]<<" "<<forces[0]<<" "<<forces[1]<<" "<<gsl_matrix_get(R,0,0) << " " << gsl_matrix_get(R,0,1) << " " << gsl_matrix_get(R,0,2) << " " << gsl_matrix_get(R,1,0) << " " << gsl_matrix_get(R,1,1) << " " << gsl_matrix_get(R,1,2)<<" "<<deg_1<<" "<<deg_2<<std::endl;
		//
...
void forceTune(UrDriver *ur3, std::condition_variable *rt_msg_cond_, int run_time, double f_ref, double t_ref)
{
	pthread_t forceID;
	startFT(&forceID);	
	...
	strcpy(file_name,"/home/xiaoli/Documents/xiaoli/data_xiaoli/logs/forceTunelog");
	strcat(file_name,sTemp);
	forcelog.open(file_name,std::ofstream::app);
    //forcelog.open("/home/xiaoli/Documents/xiaoli/data_xiaoli/logs/forceTunelog", std::ofstream::app);
	...
	//=======================================================================
	//double references[6] = {0,0,0,0,0,0}; //ref_Fx, ref_Fy, ref_Fz, ref_Tx, ref_Ty, ref_Tz
	double references[6] = {0,0,0,0,-1,0};
        double speed[6] = {0,0,0,0,0,0};
	double vw[6] = {0,0,0,0,0,0};
	...
	//Kp=0.005;// Prefered between [0.004-0.005]
	//Ki=0.000002;// Prefered between [0.000002-0.0000022]
	//Kd=0.00005;
	
	Kp_T = 0.12;// Prefered between [0.4-0.5]
	Ki_T = 0.000005; // Not prefered due to steady-state error.
	//Kd_T = 0; // Not prefered due to noise amplification.
	...
	while(i<iter)
	{
		...
		radius_current = sqrt(pow(gsl_vector_get(O,0), 2.0)+pow(gsl_vector_get(O,1), 2.0));
   		...
   		//FORCE ERROR UPDATES
   		/*if(fabs(forces[0]) < 3 && fabs(forces[1]) < 3 && fabs(forces[2]) < 3) // Dead-band filter, cut-off at 1N
   		{
			error_Fx = error_Fx/1.2; //Gentle step-down
			error_Fy = error_Fy/1.2;
			error_Fz = error_Fz/1.2;
...
*/
		...
		vw[4] = u_Ty;
		vw[5] = u_Tz;
		//vw[6]=[u_Fx, u_Fy, u_Fz, u_Tx, u_Ty, u_Tz];
